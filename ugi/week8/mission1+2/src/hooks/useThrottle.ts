...
function useThrottle<T>(value: T, delay = 500): T {
  ...
  // useRef 사용하면 컴포포넌트가 리렌더링 되어도 값이 유지되고, 변경되어도 리렌더링을 트리거 하지 않음
  ...
  useEffect(() => {
    ...
    if (Date.now() >= lastExecuted.current + delay) {
      ...
      const timerId = setTimeout(() => {
        ...
        lastExecuted.current = Date.now();
        // 최신 value를 throttledValue에 저장해서 컴포포넌트 리렌더링
        setThrottledValue(value);
      }, delay);

      // CleanUp Function 이펙트가 재실행되기 전에 타이머가 실행되지 않았다면
      // 기존 타이머를 clearTimeout을 통해 취소하여 중복 업뎃뎃를 방지함
