...
const MovieDetailPage: FC = () => {
  ...
  const { data: movie, isLoading: movieLoading, isError: movieError, error: movieErrorMessage } = 
    useCustomFetch<Movie>(movieUrl);
  
  const { data: credits, isLoading: creditsLoading, isError: creditsError, error: creditsErrorMessage } = 
    useCustomFetch<Credit>(creditsUrl);
  ...
  if (isError || !movie || !credits) {
    return <ErrorMessage message={errorMessage || '영화 정보를 찾을 수 없습니다.'} />;
  }
