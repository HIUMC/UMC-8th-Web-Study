...
export default function MovieCard({movie, onClick}: MovieCardProps) {
  ...
  return (
    <Link to={`/movie/${movie.id}?language=en-US`}>
      <div 
      className='relative rounded-xl shadow-lg overflow-hidden cursor-pointer transition-transform transform hover:scale-105' onClick={() => onClick && onClick(movie.id)}
