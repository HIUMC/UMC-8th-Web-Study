...
export default function LpPage() {
  ...
  } = useLpInfiniteList(order,debouncedSearch);

  


  useEffect(() => {
    const handler = setTimeout(() => {  // 입력이 바뀔 때마다
  ...
  const observer = new IntersectionObserver(entries => {
    if (entries[0].isIntersecting && hasNextPage) {
      throttledFetchNextPage();  // 원래 fetchNextPage() 대신 이걸 호출
    }
  }, { threshold: 1 });

  if (loadMoreRef.current) observer.observe(loadMoreRef.current);
  return () => observer.disconnect();
}, [throttledFetchNextPage, hasNextPage]);
    ...
    <div className="flex bg-gray-950 min-h-screen">
      ...
      <aside className="hidden md:block w-64 bg-gray-900 border-r border-gray-800 p-6">
        ...
        <ul className="space-y-4 text-gray-300">
          ...
          <li><button type = "button" onClick={() => setShowSearch(prev => !prev)}  className="w-full text-left px-2 py-1 rounded hover:bg-gray-800 hover:text-blue-400 transition-colors">찾기</button>
            {showSearch && (
              <div className="mt-2">
                <input type="text" placeholder="검색어를 입력하세요" className="w-full px-3 py-2 bg-gray-800 text-gray-300 rounded focus:outline-none focus:ring focus:ring-blue-500" 
...
``
        <button type="button"className="fixed bottom-8 right-8 bg-blue-600 text-white rounded-full w-12 h-12 flex items-center justify-center shadow-lg hover:bg-blue-500 transition" onClick={handleOpenModal}>+</button>
      </section>

      {isModalOpen && <AddLpModal isOpen={isModalOpen} onClose={handleCloseModal} />}
    </div>
  );
}